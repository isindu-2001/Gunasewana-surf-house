...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['booking_id'])) {
    ...
    if (isset($_POST['payment_status'])) {
        ...
        if ($booking_id <= 0 || !in_array($payment_status, ['paid', 'pending', 'failed'])) {
            echo json_encode(['success' => false, 'message' => 'Invalid input']);
            exit;
        }
        
        $query = "UPDATE `bookings_garden` SET `payment_status` = ? WHERE `booking_id` = ?";
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'delete') {
        $query = "DELETE FROM `bookings_garden` WHERE `booking_id` = ? AND `booking_status` = 'cancelled'";
        $stmt = mysqli_prepare($conn, $query);
...
$conn = db_connect();
$query = "SELECT `booking_id`, `user_id`, `event_id`, `checkin`, `checkout`, `guests`, `booking_status`, `payment_status`, `bookingOn` FROM `bookings_garden` WHERE 1";
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4">Garden Bookings Payment Management</h2>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            ...
                            while($row = mysqli_fetch_assoc($result)) { 
                                $event = $events->getEvent($row['event_id']);
                            ...
                            <tr>
                                ...
                                <td><?=date('d M Y H:i', strtotime($row['bookingOn']))?></td>
                                <td>
                                    ...
                                    <?php else: ?>
                                        <select class="form-select payment-status" data-booking-id="<?=$row['booking_id']?>" style="width: 120px;">
    ...
    $(document).ready(function() {
        ...
        $('.payment-status').on('change', function() {
            ...
            const $badge = $(this).closest('tr').find('.badge').eq(1); 
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        const badgeClass = newStatus === 'paid' ? 'bg-success' :
                                        newStatus === 'pending' ? 'bg-warning' :
                                        newStatus === 'failed' ? 'bg-danger' : 'bg-secondary';
